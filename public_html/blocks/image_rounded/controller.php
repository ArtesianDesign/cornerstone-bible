<?php 
	...
	class ImageRoundedBlockController extends BlockController {

		protected $btInterfaceWidth = 300;
		protected $btInterfaceHeight = 440;
		protected $btTable = 'btContentImageRounded';

		/** 
		 * Used for localization. If we want to localize the name/description we have to include this
		 */
		public function getBlockTypeDescription() {
			return t("Adds images (with rounded corners) and onstates from the library to pages.");
		}
		
		public function getBlockTypeName() {
			return t("Rounded Image");
		}		
		...
		function getContentAndGenerate($align = false, $style = false, $id = null) {
			...
			$sizeStrStyle = ' width:' . $size[0] . 'px; height:' . $size[1] . 'px;';
			
			$img = "<div class=\"rounded\" style=\"{$sizeStrStyle} background:url({$relPath}); \">";
			$img .= "<span>{$this->altText}</span>";
			/*$img .= "<img border=\"0\" class=\"ccm-image-block absolute\" alt=\"{$this->altText}\" src=\"{$relPath}\" {$sizeStr} ";
